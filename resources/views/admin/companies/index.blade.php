...
<div class="w-full border p-4 m-4">
    ...
    <table class="table table-striped">
        ...
        <tbody>
            ...
            <div class="modal fade" id="infoEstablecimientoModal{{ $company->id }}" tabindex="-1" aria-labelledby="infoEstablecimientoModalLabel" aria-hidden="true">
                <div class="modal-dialog">
                    <div class="modal-content">
                        <form id="establecimientoForm" method="POST" action="{{ route('establecimiento.store', $company->id) }}">
                            @csrf
            ...
            <!-- MODAL VER DETALLE -->
            <div class="modal fade" id="modal{{ $company->id }}" tabindex="-1" aria-labelledby="exampleModalLabel"
                aria-hidden="true">
...
<script>
    document.getElementById('establecimientoForm').addEventListener('submit', function(e) {
        const inputs = document.querySelectorAll('#establecimientoForm input[required]');
        let valid = true;

        inputs.forEach(input => {
            if (!input.value.trim()) {
                valid = false;
                input.classList.add('is-invalid'); // Añade una clase de error
            } else {
        ...
        if (!valid) {
            e.preventDefault(); // Detiene el envío si hay campos vacíos
